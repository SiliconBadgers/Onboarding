...
class Machine:
    ...
    def _exec_matmul(self, insn: Instruction) -> None:
        ...
        for row in range(n):
            total = 0
            for col in range(k):
                total += int(self.spad_a[a + col]) * int(self.spad_w[w + row * k + col])
            if accumulate:
                total += int(self.acc[acc + row])
            self.acc[acc + row] = wrap_i32(total)
        self.cycles += n * k
    ...
    def _exec_add_bias(self, insn: Instruction) -> None:
        ...
        for i in range(count):
            self.acc[acc + i] = wrap_i32(
                int(self.acc[acc + i]) + int(self.spad_b[bias + i])
            )
        self.cycles += count
    ...
    def _exec_relu(self, insn: Instruction) -> None:
        ...
        for i in range(count):
            value = int(self.acc[acc + i])
            if relu and value < 0:
                value = 0
            self.spad_a[dst + i] = clamp_i8(value >> shift)
        self.cycles += count
