...
module cub_core #(
    ...
    always_comb begin
        ...
        // --- SOLUTION(stage=6): set running_sum_next to running_sum plus the product of the two bytes coming out of the scratchpads this cycle, activation_read_data and weight_read_data. Both are already declared `logic signed`, so a plain `*` between them is a signed multiply -- do not cast either one, and do not introduce an unsigned intermediate. ---
        running_sum_next = running_sum + (activation_read_data * weight_read_data);
        // --- END SOLUTION ---
    end
    ...
    always_comb begin
        ...
        // --- SOLUTION(stage=6): starting from rectify_value = rectify_input, do three things. (1) If flag_bit is set and rectify_input is negative (its bit 31 is 1), set rectify_value to zero. (2) Shift rectify_value right by field_rectify_shift[4:0] using the arithmetic shift `>>>`, not `>>`, so negative values stay negative. (3) Set rectify_output to rectify_value clamped: 127 if it is above 127, -128 if it is below -128, otherwise its low 8 bits. ---
        if (flag_bit && rectify_input[31])
            rectify_value = 32'sd0;
        rectify_value = rectify_value >>> field_rectify_shift[4:0];
        if (rectify_value > 32'sd127)
            rectify_output = 8'sd127;
        else if (rectify_value < -32'sd128)
            rectify_output = -8'sd128;
        else
            rectify_output = rectify_value[7:0];
        // --- END SOLUTION ---
    end
